...
const Signup = () => {
  ...
  return (
    <div style={{ maxWidth: '400px', margin: '100px auto', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', borderRadius: '8px' }}>
      <h2 style={{ textAlign: 'center', color: '#0984e3' }}>Create Account</h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" disabled={loading} style={{ width: '100%', padding: '12px', backgroundColor: '#0984e3', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>{loading ? 'Creating Account...' : 'Sign Up'}</button>
      </form>
      <p style={{ textAlign: 'center', marginTop: '20px' }}>Already have an account? <Link to="/login" style={{ color: '#0984e3' }}>Login</Link></p>
    </div>
  );
};
